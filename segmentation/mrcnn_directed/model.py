...
from segmentation.mrcnn_directed import utils
...
def load_image_gt(dataset, config, image_id, augmentation=None, use_mini_mask=True):
...
class MaskRCNN():
    ...
    def __init__(self, mode, config, model_dir, resnet_only=False):
        ...
        self.set_log_dir()
        if resnet_only:
            self.keras_model = self.build_resnet_only(mode=mode, config=config)
        else:
            self.keras_model = self.build(mode=mode, config=config)

    def build_resnet_only(self, mode, config):
        assert mode in ['training', 'inference']
        ...
        model = KM.Model(input_image, C5, name='resnet')
        return model
